...
import { isSecureDeployment, platformAuthCookieOptions } from "./cookie-options";

/**
 * The Platform auth cookie must stay **host-scoped**.
 *
...
const MANAGED = ["NODE_ENV", "NEXT_PUBLIC_PLATFORM_APP_URL"] as const;
...
test("no Domain attribute is ever configured", () => {
  for (const env of [
    { NODE_ENV: "production", NEXT_PUBLIC_PLATFORM_APP_URL: "https://app.signalthread.ai" },
    { NODE_ENV: "development", NEXT_PUBLIC_PLATFORM_APP_URL: "http://localhost:3001" },
    {},
  ]) {
    withEnv(env, () => {
      const options = platformAuthCookieOptions();
...
test("the parent domain is never named anywhere in the options", () => {
  withEnv({ NODE_ENV: "production", NEXT_PUBLIC_PLATFORM_APP_URL: "https://app.signalthread.ai" }, () => {
    const serialized = JSON.stringify(platformAuthCookieOptions());
...
test("production over HTTPS marks the cookie Secure", () => {
  withEnv({ NODE_ENV: "production", NEXT_PUBLIC_PLATFORM_APP_URL: "https://app.signalthread.ai" }, () => {
    ...
    assert.equal(platformAuthCookieOptions().secure, true);
  });
});
...
test("local HTTP development does not mark the cookie Secure", () => {
  ...
  withEnv({ NODE_ENV: "development", NEXT_PUBLIC_PLATFORM_APP_URL: "http://localhost:3001" }, () => {
    assert.equal(isSecureDeployment(), false);
    assert.equal(platformAuthCookieOptions().secure, false);
  });
});
...
test("a local production build over HTTP still receives a usable cookie", () => {
  ...
  withEnv({ NODE_ENV: "production", NEXT_PUBLIC_PLATFORM_APP_URL: "http://localhost:3001" }, () => {
    assert.equal(isSecureDeployment(), false);
  });
});

test("a malformed app URL falls back to NODE_ENV rather than silently disabling Secure", () => {
  withEnv({ NODE_ENV: "production", NEXT_PUBLIC_PLATFORM_APP_URL: "not-a-url" }, () => {
...
test("removal uses the same scope as writing", () => {
  ...
  withEnv({ NODE_ENV: "production", NEXT_PUBLIC_PLATFORM_APP_URL: "https://app.signalthread.ai" }, () => {
    const a = platformAuthCookieOptions();
    const b = platformAuthCookieOptions();
