...
/**
 * Cookie contract for Orca's Platform Core auth session.
 *
 * The session cookie is deliberately **host-scoped**: no `Domain` attribute is
 * ever set, so a cookie written by `orca.signalthread.ai` is not sent to
 * `app.signalthread.ai` or any sibling product. That is the intended architecture, not an oversight.
...
export function isSecureDeployment(): boolean {
  const appUrl = process.env.NEXT_PUBLIC_ORCA_APP_URL?.trim();
  if (appUrl) {
    try {
 ...
 */
export function orcaAuthCookieOptions(): CookieOptions {
  return {
    path: "/",
    sameSite: "lax",
