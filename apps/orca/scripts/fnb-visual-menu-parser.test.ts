...
describe("pdf-parse worker resolution", () => {
  // Regression guard for the monorepo move. The worker path used to be built from
  // process.cwd(), which only held while the app owned its own node_modules. Under npm
  // workspaces pdf-parse hoists to the repository root, so a cwd-relative path silently
  // misses the file and PDF menu parsing fails at runtime with no build or type error.
  it("resolves the worker bundle wherever pdf-parse is installed", () => {
    const workerUrl = resolvePdfWorkerUrl();
    const workerPath = fileURLToPath(workerUrl);

    assert.ok(workerUrl.startsWith("file://"), "worker must be a file:// URL for setWorker");
    assert.ok(existsSync(workerPath), `worker bundle missing on disk: ${workerPath}`);
    assert.equal(path.basename(workerPath), "pdf.worker.mjs");
    assert.ok(
      workerPath.includes(`${path.sep}pdf-parse${path.sep}`),
      "worker must resolve inside the pdf-parse package",
    );
  });

  it("does not depend on the process working directory", () => {
    // The old implementation changed answer with cwd; the resolver must not.
    const fromAppRoot = resolvePdfWorkerUrl();
    const originalCwd = process.cwd();
    try {
      process.chdir(path.parse(originalCwd).root);
      assert.equal(resolvePdfWorkerUrl(), fromAppRoot);
      assert.ok(existsSync(fileURLToPath(resolvePdfWorkerUrl())));
    } finally {
