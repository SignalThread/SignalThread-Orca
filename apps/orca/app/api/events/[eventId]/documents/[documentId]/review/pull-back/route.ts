...
import { DocumentServiceError, pullBackDocumentReview } from "@/src/server/services/documents";
...
async function postHandler(
  ...
  try {
    ...
    return toErrorResponse(error, "POST /api/events/:eventId/documents/:documentId/review/pull-back");
  }
  ...
  try {
    const payload = await pullBackDocumentReview(eventId, documentId, {
      note: body.note,
...
export const POST = withApiRequestLogging("POST /api/events/:eventId/documents/:documentId/review/pull-back", postHandler);
