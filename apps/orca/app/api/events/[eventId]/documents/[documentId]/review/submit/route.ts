...
import { DocumentServiceError, submitDocumentForReview } from "@/src/server/services/documents";
...
async function postHandler(
  ...
  try {
    ...
    return toErrorResponse(error, "POST /api/events/:eventId/documents/:documentId/review/submit");
  }
  ...
  try {
    const payload = await submitDocumentForReview(eventId, documentId, {
...
export const POST = withApiRequestLogging("POST /api/events/:eventId/documents/:documentId/review/submit", postHandler);
