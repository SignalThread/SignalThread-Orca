...
import { cancelScheduledEmailSend } from "@/src/server/services/marketing";
...
async function postHandler(
  ...
  try {
    ...
    const send = await cancelScheduledEmailSend(auth.user, sendId);
    return NextResponse.json(send);
  } catch (error) {
    return toMarketingRouteErrorResponse(error, "POST /api/events/:eventId/marketing/sends/:sendId/cancel");
  }
}

export const POST = withApiRequestLogging(
  "POST /api/events/:eventId/marketing/sends/:sendId/cancel",
  postHandler,
);
