...
import { retryFailedEmailSend } from "@/src/server/services/marketing";
...
async function postHandler(
  ...
  try {
    ...
    const result = await retryFailedEmailSend(auth.user, sendId);
    return NextResponse.json(result);
  } catch (error) {
    return toMarketingRouteErrorResponse(error, "POST /api/events/:eventId/marketing/sends/:sendId/retry");
  }
}

export const POST = withApiRequestLogging(
  "POST /api/events/:eventId/marketing/sends/:sendId/retry",
  postHandler,
);
