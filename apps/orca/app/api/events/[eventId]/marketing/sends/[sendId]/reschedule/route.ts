...
import { rescheduleEmailSend } from "@/src/server/services/marketing";
...
async function postHandler(
  ...
  try {
    ...
    const send = await rescheduleEmailSend(auth.user, sendId, body);
    return NextResponse.json(send);
  } catch (error) {
    return toMarketingRouteErrorResponse(error, "POST /api/events/:eventId/marketing/sends/:sendId/reschedule");
  }
}

export const POST = withApiRequestLogging(
  "POST /api/events/:eventId/marketing/sends/:sendId/reschedule",
  postHandler,
);
