...
async function postHandler(
  ...
  try {
    const submission = await decideBudgetSubmission(eventId, submissionId, "REJECTED", auth.user.id);
    return NextResponse.json(submission);
  } catch (error) {
    return toErrorResponse(error, "POST /api/events/:eventId/budget/submissions/:submissionId/reject");
  }
}

export const POST = withApiRequestLogging("POST /api/events/:eventId/budget/submissions/:submissionId/reject", postHandler);
