...
import { completeTask, getTask } from "@/src/server/services/tasks";
...
async function postHandler(
  ...
  try {
    ...
    const task = await completeTask(auth.user, taskId);
    return NextResponse.json(task);
  } catch (error) {
    return toTaskRouteErrorResponse(error, "POST /api/events/:eventId/tasks/:taskId/complete");
  }
}

export const POST = withApiRequestLogging("POST /api/events/:eventId/tasks/:taskId/complete", postHandler);
