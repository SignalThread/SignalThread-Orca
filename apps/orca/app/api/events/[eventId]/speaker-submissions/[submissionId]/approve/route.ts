...
async function postHandler(
  ...
  try {
    const submission = await approveSpeakerSubmission(eventId, submissionId, authResult.user);
    return NextResponse.json(submission);
  } catch (error) {
    return toErrorResponse(error, "POST /api/events/:eventId/speaker-submissions/:submissionId/approve");
  }
}

export const POST = withApiRequestLogging(
  "POST /api/events/:eventId/speaker-submissions/:submissionId/approve",
  postHandler,
);
