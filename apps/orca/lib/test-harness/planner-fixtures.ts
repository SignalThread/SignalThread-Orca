...
export function loadPlannerTestEnv(): void {
  // The app lives at <repo>/apps/orca, so the repository root is two levels up. Before the
  // monorepo move this climbed a single level from web/; keeping that would have silently
  // stopped loading the repo-root .env.local and left DATABASE_URL unset in DB-backed tests.
  const appRoot = resolve(process.cwd());
  const repoRoot = resolve(appRoot, "..", "..");
  loadLocalEnvFile(join(repoRoot, ".env.local"));
  loadLocalEnvFile(join(appRoot, ".env.local"));
}
