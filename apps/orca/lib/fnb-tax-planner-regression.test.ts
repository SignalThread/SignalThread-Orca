...
const migration = readFileSync("test-fixtures/legacy-orca-migrations/20260710120000_add_session_fnb_tax_configuration/migration.sql", "utf8");
...
test("mirrored migration is additive, defaults existing sessions to zero, and cascades item taxes", () => {
  assert.match(migration, /ADD COLUMN "fnbTaxPercent" DECIMAL\(7,4\) NOT NULL DEFAULT 0/);
  assert.match(migration, /ADD COLUMN "fnbServiceChargePercent" DECIMAL\(7,4\) NOT NULL DEFAULT 0/);
  assert.match(migration, /CREATE TABLE "SessionFnbCatalogAssignmentTax"/);
