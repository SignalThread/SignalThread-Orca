...
const CAPABILITIES = [
  ["mailSend", "Send Outlook follow-ups"],
  ["calendarReadWrite", "Manage calendar events"]
] as const;

export function Microsoft365ConnectionPanel({
  status
}: {
  ...
  const needsReconnect =
    status.status === "reconnect_required" || status.status === "error" || status.isPartialGrant;
  const connectHref = needsReconnect
    ? "/api/exhibitor/integrations/microsoft/connect?reconnect=1"
    : "/api/exhibitor/integrations/microsoft/connect";

  const disconnect = () => {
    startTransition(async () => {
      setError(null);
      try {
        const response = await fetch("/api/exhibitor/integrations/microsoft/disconnect", {
          method: "POST",
        ...
        if (!response.ok || !payload.success || !payload.disconnected) {
          setError(payload.error ?? "Unable to disconnect Microsoft 365.");
          return;
        }
        setDialogOpen(false);
        router.refresh();
      } catch {
        setError("Unable to disconnect Microsoft 365. Try again in a moment.");
      }
    });
  };
    ...
    <article className="rounded-3xl border border-slate-200 bg-white p-7 shadow-[0_14px_38px_rgba(15,23,42,0.06)]">
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-3xl font-bold text-slate-950">Microsoft 365</h2>
      ...
      {status.identity ? (
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.1em] text-slate-500">Connected Microsoft account</p>
      ...
      {status.isPartialGrant ? (
        <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          Microsoft did not grant every requested permission. Reconnect and approve the requested access.
        </p>
      ) : null}
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {status.status === "disconnected" || needsReconnect ? (
          <a href={connectHref} className="inline-flex h-11 items-center justify-center rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 px-5 text-sm font-semibold text-white shadow-sm hover:from-indigo-600 hover:to-violet-700">
            {status.status === "disconnected" ? "Connect Microsoft 365" : "Reconnect Microsoft 365"}
          </a>
        ) : null}
        {canDisconnect ? (
      ...
      {dialogOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4" role="dialog" aria-modal="true" aria-labelledby="microsoft-disconnect-title">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
            <h3 id="microsoft-disconnect-title" className="text-xl font-bold text-slate-950">Disconnect Microsoft 365?</h3>
            <p className="mt-3 text-sm leading-6 text-slate-600">
              SignalThread removes the encrypted credentials immediately, so no further Microsoft access is possible. Microsoft has no per-app token revocation endpoint, so remove SignalThread from myapps.microsoft.com if you also want the consent entry cleared there. No lead data is changed.
            </p>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <button type="button" disabled={isPending} onClick={disconnect} className="h-10 rounded-lg bg-rose-600 px-4 text-sm font-semibold text-white disabled:opacity-60">{isPending ? "Disconnecting..." : "Disconnect"}</button>
            </div>
          </div>
        </div>
      ) : null}
