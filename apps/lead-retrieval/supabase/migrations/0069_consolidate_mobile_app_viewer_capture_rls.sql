-- Mobile app–scoped RLS only (from app migration 0009). Admin/dashboard RBAC is unchanged (see 0064 users_role_check).
-- exhibitor_viewer with event_users.permissions granting app access can SELECT / INSERT / UPDATE leads for the matching event and company. No DELETE. No role promotion.

BEGIN;

-- Remove legacy RPC if a prior revision of this migration was applied.
DROP FUNCTION IF EXISTS public.claim_exhibitor_admin_for_app_capture();
