...
import { removeEnrichmentIntegration } from "@/lib/integrations/enrichment-provider-config";

const REDIRECT_BASE = "/exhibitor/integrations/people-data-labs";
...
export async function POST(request: Request) {
  ...
  const { error } = await removeEnrichmentIntegration({
  ...
  if (error) {
    if (process.env.NODE_ENV !== "production") {
      return redirectTo(request, { error: "disable_failed", reason: error.message ?? "" });
    }
