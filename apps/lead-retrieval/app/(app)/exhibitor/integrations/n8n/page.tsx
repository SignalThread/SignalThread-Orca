...
export default async function ExhibitorN8nIntegrationPage({
  searchParams,
}: {
    ...
    return (
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">n8n Integration</h1>
            <p className="text-slate-600">Configure webhook delivery for n8n automation scenarios.</p>
          </div>
          <Link
        ...
        <p className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700">
          Your exhibitor admin user is not linked to an account, so n8n setup cannot be saved.
        </p>
      </section>
    );
  }
  ...
  const integration = (data as WebhookIntegrationRow | null) ?? null;
  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">n8n Integration</h1>
          <p className="text-slate-600">Configure outbound webhook payloads for your n8n automations.</p>
        </div>
        <Link
          href="/exhibitor/integrations"
          className="inline-flex h-10 items-center justify-center rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_1px_2px_rgba(15,23,42,0.06)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              {isConfigured
                ? isEnabled
                  ? "n8n integration is configured and enabled."
                  : "n8n integration is configured but currently disabled."
                : "Set up your n8n webhook destination to enable this integration."}
            </p>
          </div>
          <span
        ...
        <dl className="mt-4 grid grid-cols-1 gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-2">
          <div>
            ...
            <dd className="mt-1 text-sm font-semibold text-slate-900">n8n</dd>
          </div>
          <div>
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_1px_2px_rgba(15,23,42,0.06)]">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-xl font-semibold text-slate-900">n8n setup</h3>
          <span
        ...
        {showSetupForm ? (
          <form method="post" action="/api/exhibitor/integrations/n8n/setup" className="mt-5 space-y-6">
            ...
            <fieldset className="space-y-3">
              ...
              <p className="text-xs text-slate-500">
                Choose how much data n8n receives. Each option is cumulative from top to bottom.
              </p>
            ...
            <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
              ...
              <span className="space-y-1 text-sm text-slate-700">
                ...
                <span className="block text-xs text-slate-600">
                  When enabled, SignalThread Lead Retrieval can send n8n webhook payloads using your saved configuration.
                </span>
              </span>
            </label>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Link
                href="/exhibitor/integrations/n8n?edit=1"
              ...
              {isEnabled ? (
                <form method="post" action="/api/exhibitor/integrations/n8n/disable">
                  <button
                    type="submit"
                ...
                : resolvedSearchParams.error === "missing_account"
                  ...
                  : resolvedSearchParams.error === "save_failed"
                    ...
                    : resolvedSearchParams.error === "disable_failed"
                      ? `Failed to disable n8n integration.${
                          process.env.NODE_ENV !== "production" && resolvedSearchParams.reason
                            ? ` Reason: ${resolvedSearchParams.reason}`
                            : ""
                        }`
                      : "n8n setup request failed."}
        </p>
      ) : null}
    </section>
  );
