...
/**
 * Pulse -> Platform: claim a one-time launch handoff.
 *
 * This is the ONLY place Pulse talks to the SignalThread Platform at runtime, and
 * it is a plain HTTPS call to Platform's own API. Pulse never holds Platform Core
 * Auth credentials (not even the anon key), never opens a Platform Core session,
 * and never reads Platform's database. The token the browser arrived with is
 * handed back to Platform, which exchanges it exactly once against Platform Core
 * Auth and answers with the canonical user / organization / event it re-derived
 * from live registry state.
 *
 * What comes back is context, not authorization. Pulse still resolves each id
 * through its own mapping columns and applies its own access model afterwards.
 */

export const PULSE_PRODUCT_KEY = 'pulse'
...
const fail = (reason: ClaimFailureReason, platformReason?: string): ClaimResult =>
  ({ ok: false, reason, ...(platformReason ? { platformReason } : {}) })
 ...
 * response is what Pulse trusts for identity, so it may only travel over TLS to
 * the host Pulse was configured with.
 */
 ...
 *
 * The correlator is `SHA-256(nonce)`, not the nonce itself: Platform relays it
 * back on the handoff redirect so Pulse can prove the returning browser started
 * the launch, and it is useless to anyone who intercepts the URL.
 */
export function buildPlatformLaunchUrl(
  ...
  const url = new URL(`${base}/api/launch/${PULSE_PRODUCT_KEY}`)
...
/**
 * Resolve `work` unless `signal` aborts first. A manually constructed Response is
 * not tied to the fetch signal, and even a real one can stall while streaming its
 * body, so body consumption is raced against the same abort that bounds the
 * request: the timeout covers headers, body and JSON parsing as one budget.
 */
...
export async function claimPlatformHandoff(
  ...
  try {
    const response = await untilAbort(
      doFetch(`${base}/api/launch/${PULSE_PRODUCT_KEY}/claim`, {
        method: 'POST',
    ...
    // The abort budget stays armed until the body is fully read and parsed.
  ...
  if (product !== PULSE_PRODUCT_KEY) return fail('PLATFORM_UNAVAILABLE')
