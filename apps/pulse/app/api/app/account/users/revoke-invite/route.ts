...
import { revokeAccountInvite } from '@/lib/account-users'
...
export async function POST(request: NextRequest) {
  ...
  const result = await revokeAccountInvite({
    accountId: auth.account.id,
    email: parsed.data.email,
  })

  if (!result.ok) {
    return NextResponse.json({ success: false, error: 'not_found', message: result.message }, { status: result.status })
  }
