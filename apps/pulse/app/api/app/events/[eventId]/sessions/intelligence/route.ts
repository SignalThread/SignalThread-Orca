...
export async function GET(request: NextRequest, { params }: { params: { eventId: string } }) {
  ...
  try {
    const data = await getEventSessionIntelligence({
    ...
    console.error('[GET /api/app/events/[eventId]/sessions/intelligence]', error)
    return NextResponse.json({ success: false, error: 'Failed to load session intelligence' }, { status: 500 })
  }
}
