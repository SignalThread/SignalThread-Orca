...
const briefDocumentSource = fs.readFileSync(path.join(process.cwd(), 'components/events/EventClosingBriefDocument.tsx'), 'utf8')
...
describe('EventPostEventClosingBrief', () => {
  it('uses the shared Event Overview hierarchy above the retained post-event findings', () => {
    ...
    expect(source).not.toContain('data-post-event-metrics')
    expect(source).not.toContain('lg:grid-cols-[minmax(0,2.08fr)_minmax(310px,0.92fr)]')
    expect(source).not.toContain('data-brief-subsection')
    expect(source).not.toContain('Closing brief · {')
  ...
  it('links the report to one dedicated brief document workflow instead of printing the dashboard', () => {
    ...
    expect(briefActionSource).toContain('const generateBrief = async (forceRefresh = false) =>')
    expect(briefActionSource).not.toContain('cacheBust=${Date.now()}')
    expect(briefActionSource).toContain('setBriefHash(')
    expect(briefActionSource).toContain('lifecycle: lifecyclePhase, briefHash')
    expect(briefDocumentSource).toContain('pdfHref')
    expect(briefActionSource).toContain('const briefDataHref =')
    expect(briefActionSource).toContain('lifecycle: lifecyclePhase')
    expect(briefActionSource).toContain('Generate brief')
    expect(briefActionSource).toContain('View brief')
    expect(briefDocumentSource).toContain('EventBriefPdfDownloadButton')
    expect(briefDocumentSource).toContain('Regenerate brief')
