...
Model routing is a control-plane concern, not something Markdown can enforce by itself. Before starting Prompt 0, determine whether the host supports selecting a model for the primary agent and/or subagents.

Use these portable tier names unless the prompt pack defines provider-specific model IDs:

```txt
HIGH_REASONING       architecture, root-cause analysis, authorization, schema, data integrity, complex implementation
STRONG_CODING        UI implementation, ordinary feature work, focused refactors, integration work
FAST_VERIFICATION    repetitive tests, regression audits, documentation, evidence collection
```

The prompt pack may map these tiers to concrete model IDs. If it does not, use the host's strongest available coding/reasoning model for implementation and a faster suitable model only for independent verification.

Rules:

1. Select or request the model at the **prompt boundary**, before editing begins.
2. A prompt may not switch models midway through implementation unless the host explicitly supports it and the transition is recorded.
3. Subagents may use a different tier only when the active prompt authorizes subagents and their work is disjoint.
4. Never claim that a model changed unless the host returned or displayed confirmation. If model selection is unavailable, record `UNAVAILABLE — host retained active model` and continue using the active model when safe.
5. A model request must never override scope, plan authority, schema mode, safety rules, or stop gates.
6. Record the actual model, requested tier, subagents/models, and any fallback in every prompt summary.

Required startup record:

```txt
Model-routing capability: supported / unavailable / unknown
Primary model actually used:
Requested tier:
Subagent models actually used:
...
Before step 4, apply the active prompt's model-routing assignment and record whether it was actually honored.
...
Model routing policy:
[tier-to-model mapping, or `use portable tiers`]
