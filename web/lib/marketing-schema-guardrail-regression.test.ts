...
const WEB_SCHEMA = "prisma/schema.prisma";
...
test("marketing models avoid JSON blobs for core KPI, recipient, send, and event data", () => {
  const schema = readSchema(WEB_SCHEMA);
  for (const model of MARKETING_MODELS) {
...
test("marketing models and enums are present in both Prisma schema copies", () => {
  const webSchema = readSchema(WEB_SCHEMA);
  const rootSchema = readSchema(ROOT_SCHEMA);
  for (const model of MARKETING_MODELS) {
    assert.ok(webSchema.includes(`model ${model} {`), `web schema must define ${model}`);
    assert.ok(rootSchema.includes(`model ${model} {`), `root schema must define ${model}`);
  }
  for (const enumName of MARKETING_ENUMS) {
    assert.ok(webSchema.includes(`enum ${enumName} {`), `web schema must define ${enumName}`);
    assert.ok(rootSchema.includes(`enum ${enumName} {`), `root schema must define ${enumName}`);
  }
});
