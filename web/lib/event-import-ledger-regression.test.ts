...
const rootMigration = readFileSync("../prisma/migrations/20260806120000_event_import_ledger/migration.sql", "utf8");
const webMigration = readFileSync("prisma/migrations/20260806120000_event_import_ledger/migration.sql", "utf8");
...
test("both Prisma schema copies and additive migrations carry the same import ledger contract", () => {
  ...
  assert.ok(rootMigration.includes("ADD VALUE IF NOT EXISTS 'VENDOR'"));
  assert.ok(rootMigration.includes('CREATE TABLE "EventImportIntent"'));
  assert.ok(rootMigration.includes('CREATE TABLE "EventImportResult"'));
  assert.ok(!rootMigration.match(/(?:^|\n)\s*(?:DROP|TRUNCATE|DELETE)\b/));
});
