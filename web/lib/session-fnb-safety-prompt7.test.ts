...
test("Prompt 7 safety API and workspace enforce read/write boundaries and privacy guidance", async () => {
  const [route, component, readiness, rootSchema, webSchema, rootMigration, webMigration] = await Promise.all([
    ...
    readFile("prisma/schema.prisma", "utf8"),
    readFile("../prisma/migrations/20260811150000_session_safety_operations/migration.sql", "utf8"),
    readFile("prisma/migrations/20260811150000_session_safety_operations/migration.sql", "utf8"),
  ]);
  ...
  assert.match(rootSchema, /catalogItemSnapshot\s+Json\?/);
  assert.match(rootMigration, /catalogItemSnapshot/);
  assert.match(rootMigration, /verified_modification_check/);
