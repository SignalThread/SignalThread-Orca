...
import { BudgetServiceError, finalizeBudgetFileUpload } from "@/src/server/services/budget";
...
async function postHandler(
  ...
  try {
    ...
    const payload = await finalizeBudgetFileUpload(eventId, documentId, body, {
    ...
    return toErrorResponse(error, "POST /api/events/:eventId/budget/files/:documentId/finalize-upload");
  }
}

export const POST = withApiRequestLogging(
  "POST /api/events/:eventId/budget/files/:documentId/finalize-upload",
  postHandler,
);
