...
import { DocumentServiceError, rejectDocument } from "@/src/server/services/documents";
...
async function postHandler(
  ...
  try {
    ...
    return toErrorResponse(error, "POST /api/events/:eventId/documents/:documentId/reject");
  }
  ...
  try {
    const payload = await rejectDocument(eventId, documentId, {
      ...body,
...
export const POST = withApiRequestLogging("POST /api/events/:eventId/documents/:documentId/reject", postHandler);
