...
import { DocumentServiceError, reopenDocument } from "@/src/server/services/documents";
...
async function postHandler(
  ...
  try {
    ...
    return toErrorResponse(error, "POST /api/events/:eventId/documents/:documentId/reopen");
  }
  ...
  try {
    const payload = await reopenDocument(eventId, documentId, {
      ...body,
...
export const POST = withApiRequestLogging("POST /api/events/:eventId/documents/:documentId/reopen", postHandler);
