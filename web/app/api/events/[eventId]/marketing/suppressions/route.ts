...
import { listSuppressions } from "@/src/server/services/marketing";
...
async function getHandler(request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  try {
    ...
    const suppressions = await listSuppressions(auth.user, eventId);
    return NextResponse.json(suppressions);
  } catch (error) {
    return toMarketingRouteErrorResponse(error, "GET /api/events/:eventId/marketing/suppressions");
  }
}

export const GET = withApiRequestLogging("GET /api/events/:eventId/marketing/suppressions", getHandler);
