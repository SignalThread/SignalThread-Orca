...
import {
  getSpeakerReadinessOverview,
  SpeakerReadinessError,
...
async function getHandler(
  ...
  try {
    const overview = await getSpeakerReadinessOverview(eventId, authResult.user);
    return NextResponse.json(overview);
  } catch (error) {
    return toErrorResponse(error, "GET /api/events/:eventId/speaker-readiness");
  }
}

export const GET = withApiRequestLogging("GET /api/events/:eventId/speaker-readiness", getHandler);
