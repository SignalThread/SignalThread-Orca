...
async function postHandler(
  ...
  try {
    const presign = await createAdminSpeakerFilePresign(eventId, speakerId, authResult.user, {
      kind: body.kind,
    ...
    return toErrorResponse(error, "POST /api/events/:eventId/speakers/:speakerId/files/presign");
  }
}

export const POST = withApiRequestLogging(
  "POST /api/events/:eventId/speakers/:speakerId/files/presign",
  postHandler,
);
