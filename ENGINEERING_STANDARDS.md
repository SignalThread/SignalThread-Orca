...
### Product-grade UX, not mechanical UI
User-facing screens must meet a product-grade user experience bar, not merely expose data fields and actions.

The product must not ship screens that feel like raw admin scaffolding, database-shaped screens, or technically complete but product-poor implementation. Implementation should solve the user's workflow, not merely render the underlying model as forms, tables, buttons, and configuration blocks.

Product UI should be designed the way a strong product designer would approach it:
...
- workflows that guide users through decisions instead of making them decode system structure

Avoid raw CRUD layouts unless the user's actual task is best served by a table. Do not dump configuration into flat lists, accordions, dense forms, or mechanical cards without considering information architecture.

Empty states should either teach, guide, or disappear when they add no value. Do not reserve space for empty sections that make a finished product feel unfinished.

Summary screens should answer "what will happen?" quickly. They should not resemble debug output, report dumps, or internal reconciliation screens unless that is the user's real task.

Complex workflows should separate:
- overview
- preview
- issues and warnings
- editing or mapping
...
Do not cram all of these into one dense surface when the user needs staged understanding.

Responsive behavior is part of acceptance criteria, not a polish task. A user-facing screen is not done if it only works at the implementer's viewport width.
...
11. Would this screen make sense to a first-time user in under 5 seconds?
12. Is the primary action obvious?
13. Does the layout reflect the user workflow rather than the database structure?
14. Are empty states useful, or should they be hidden?
15. Does the UI remain usable at narrower widths?
16. Does this feel like a product experience, not internal tooling?

If the answer to any of these is no, the change is not finished.

## Implementation Prompt Guidance
Future implementation prompts should define the user experience standard, not only the fields or components to render.

Strong prompts should:
- state the user workflow being improved
- describe the desired user experience and decision flow
- call out when a screen should be redesigned rather than patched
- require polished, responsive UI for user-facing features
- reject "mechanically complete" as done when the resulting screen is hard to understand

For user-facing work, prompts should make clear whether the goal is:
- a faster workflow
- clearer information architecture
- better preview or confirmation
- safer error recovery
- reduced cognitive load
- a more polished module settings or dashboard experience

If a prompt only lists data fields, tables, or buttons, implementers should still evaluate whether that creates a product-grade user experience or merely exposes controls.
